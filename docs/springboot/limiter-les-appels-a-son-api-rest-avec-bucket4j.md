...
permalink: "/springboot/limiter-les-appels-a-son-api-rest-avec-bucket4j/"
tags: [back, java, spring-boot, rest, api, bucket, tutoriel]
source: "sfeir.dev"
...
- 🔒 [**Sécurité**](/springboot/security/securisez-vos-api-avec-spring-security-basic-auth/) : éviter les abus, les attaques par déni de service ou les scripts malveillants.
- ⚖️ **Équité** : garantir une répartition juste des ressources entre tous les utilisateurs.
- ⚙️ **Robustesse** : protéger les [bases de données](/springboot/migration-et-versioning-de-base-de-donnees-dans-une-application-spring-boot-flyway-vs-liquibase/) et les serveurs d’un trop grand nombre de requêtes simultanées.
...
- Si le bucket est vide, la requête est rejetée avec une [erreur 429 (Too Many Requests)](/springboot/comment-bien-gerer-ses-exceptions-dans-spring-boot/).
...
- 🛠️ Intégration simple dans une [application Spring Boot](/springboot/il-etait-une-fois-spring-boot/).
...
Plutôt que d’intégrer la logique de limitation directement dans chaque contrôleur ou d’appeler systématiquement un service, une autre approche consiste à utiliser [**Spring AOP**](/springboot/spring-aop-comprendre-la-programmation-orientee-aspect-dans-spring/). Cela permet de **centraliser** la vérification du quota et de l’appliquer à n’importe quelle méthode via une simple annotation.

### [l’annotation](/springboot/comprendre-les-annotations-dans-spring-boot-guide-et-exemples/) `@RateLimited`
...
- **Stockage distribué des buckets** : avec [Redis](/springboot/cache/mise-en-cache-avec-redis-dans-spring-boot/), [Hazelcast](https://hazelcast.com/?ref=sfeir.dev) ou [Ignite](https://ignite.apache.org/?ref=sfeir.dev), pour partager les quotas entre plusieurs instances.
