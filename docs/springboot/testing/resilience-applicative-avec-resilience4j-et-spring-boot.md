...
permalink: "/springboot/testing/resilience-applicative-avec-resilience4j-et-spring-boot/"
tags: [back, java, spring-boot, resilience, tutoriel, testing]
source: "sfeir.dev"
...
- **Tolérance aux pannes** : savoir comment l’application réagit lorsqu’une anomalie survient. C’est ce que l’on peut explorer avec des outils comme [**Chaos Monkey for Spring Boot**](/springboot/testing/introduisez-du-chaos-dans-votre-application-spring-boot/), qui injecte volontairement des pannes pour tester la robustesse d’un système.
...
Dans cet article, nous allons nous concentrer sur **Resilience4j** et voir comment l’intégrer dans une application [**Spring Boot**](/springboot/il-etait-une-fois-spring-boot/), avec des exemples concrets pour chacun de ses principaux modules.
...
L’intégration avec [Spring Boot](https://www.sfeir.dev/back/back-pourquoi-utiliser-spring-boot/) est facilitée grâce [aux annotations](/springboot/comprendre-les-annotations-dans-spring-boot-guide-et-exemples/) (`@CircuitBreaker`, `@Retry`, etc.) et une configuration centralisée via `application.properties`.
...
- Compatible avec les outils de monitoring (Actuator, [Prometheus, Grafana](/springboot/superviser-votre-application-spring-boot-grace-a-prometheus-et-grafana/)).
...
[Limiter les appels à son API REST avec Bucket4j](/springboot/limiter-les-appels-a-son-api-rest-avec-bucket4j/)
