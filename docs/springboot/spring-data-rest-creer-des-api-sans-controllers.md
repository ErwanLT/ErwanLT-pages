...
permalink: "/springboot/spring-data-rest-creer-des-api-sans-controllers/"
tags: [back, java, spring-boot, api, tutoriel]
source: "sfeir.dev"
...
Créer une [API REST](/definition/rest-definition/) en [Spring Boot](/springboot/il-etait-une-fois-spring-boot/), c'est souvent une succession de contrôleurs, DTO, services, tests, documentation. C'est robuste, mais pas toujours nécessaire.
...
Et à l'inverse : dès que le contrat API doit être très précis, [versionné](/springboot/versioning-des-api-dans-spring-boot-guide-complet/) ou fortement sécurisé, on bascule plus souvent sur des controllers et des DTO.
...
C'est particulièrement utile si on a déjà lu [Réussir sa migration de Swagger 2 à OpenApi 3](/springboot/reussir-sa-migration-de-swagger-2-a-openapi-3/).

## Ce que Spring Data REST ne fait pas bien

- **Contrat API explicite** : tout change dès qu'on renomme un repository ou un champ.
- **Versioning** : possible, mais moins naturel que dans un controller dédié. Voir [Versioning des API dans Spring Boot - Guide Complet](/springboot/versioning-des-api-dans-spring-boot-guide-complet/).
- **Validation fine** : on peut brancher la validation, mais le contrôle sur le cycle HTTP est plus limité qu'un controller classique. Voir [Validation Spring Boot - du standard au sur-mesure](/springboot/validation-spring-boot-du-standard-au-sur-mesure/).
