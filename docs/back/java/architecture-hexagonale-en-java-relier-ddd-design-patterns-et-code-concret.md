...
tags: [back, java, design pattern, architecture]
source: "sfeir.dev"
...
Au lieu d’instancier les objets métier n’importe comment, on centralise la création avec [Factory](/designPattern/creation/factory/).
...
Quand un comportement peut changer (tarification, remise, taxation), [Stratégie](/designPattern/comportementaux/strategie/) évite les `if/else` infinis.
...
Le port d’application définit un contrat ; l’infrastructure l’implémente avec [Adaptateur](/designPattern/structure/adaptateur/).
...
Une [Façade](/designPattern/structure/facade/) peut simplifier l’accès aux use cases côté client (CLI, REST, batch).
...
Ce niveau de testabilité va dans le même sens que [Maîtrisez votre architecture Spring Boot avec ArchUnit](/springboot/testing/maitrisez-votre-architecture-spring-boot-avec-archunit/) : rendre les limites de l’architecture explicites et vérifiables.
...
Si tu veux approfondir la logique derrière chaque pattern, commence par [Pourquoi utiliser les Design Patterns ?](/designPattern/pourquoi-utiliser-les-design-patterns/), puis traverse les patterns de création, structurels et comportementaux pour voir lesquels répondent à tes vrais besoins.

## Maillage interne

- [DDD - Définition](/definition/ddd-definition/)
- [Les Design Patterns](/definition/les-design-patterns/)
- [Pourquoi utiliser les Design Patterns ?](/designPattern/pourquoi-utiliser-les-design-patterns/)
